...
class AsyncCacherAdapter(CacherBase):
    __async_pool_lock = asyncio.Lock()
    ...
    async def _init_pool(self) -> None:
        ...
        try:
            if AsyncCacherAdapter.async_pool is None:
                ...
                # GET WORKERS
                # ============================================
                async with AsyncCacherAdapter.__async_pool_lock:
    ...
    async def __get_client(self):
        ...
        return await Redis(connection_pool=AsyncCacherAdapter.async_pool)
    ...
    async def related(self) -> bool:
        if self.server_client is None:
            try:
                ...
                # ============================================
                self.server_client = await self.__get_client()
            except Exception as e:
                log_t = (
                    self.log_t
