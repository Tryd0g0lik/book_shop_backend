...
class TestApiUploadFileValid:
    ...
    @pytest.mark.asyncio
    async def test_upload_small_file(self, transactional_db, fixture_clearner, fixture_session_of_user):
        ...
        group = await fixture_session_of_user.user.groups.afirst()
        ...
        try:
            ...
            response: Response = await catalog_file.acreate(requests)
            log.info("{} Got & Group {} & Response: {} ".format(log_t, group, str(response.__dict__)[:50]))
            detail_dict = json.loads((response.content).decode())
            if group.name.lower() == "editors":
                detail = detail_dict.get("detail")
                assert "denied" in detail.lower()
                assert response.status_code == status.HTTP_403_FORBIDDEN
            else:
                detail = detail_dict.get("detail")
                assert "File to small size" in detail
    ...
    @pytest.mark.asyncio
    async def test_permissions_upload_file(self, transactional_db, fixture_clearner,
        ...
        log.info("{} start test".format(log_t))
        prefix_log ="{}[{}]".format(self.PREFIX_LOG, "[test_permissions_upload_file]:")
        clearner_documents, clearner_database = fixture_clearner
        ...
        try:
            ...
            async with self.LOCK:
                for i in range(total_chunks_size):
                    ...
                    requests.data = files
                    requests.authenticators = True
                    requests.successful_authenticator = True
                    requests.session = {}
                    requests.session.__setitem__("user_id", fixture_session_of_user.user.id)
                    requests._request = requests
                    requests.METHOD = "POST"
                    ...
                    try:
                        ...
                        resp = await view.acreate(requests)
                        test_response = resp
                        log.debug("""
                        # ============================================
                        # User: {}
                        ...
                        log.debug("{} ERROR => {}  ".format(prefix_log, str(e), ))
                ...
                if group.name.lower() == "editors":
                    # Note: Methods using additional permissions.!!!
                    # Mean some roles do not have permissions
                    status_code = 403
                if test_response:
                    ...
                    assert 403 == status_code, "Allows {} status code!".format(status_code)
                # ============================================
                # CLEARING THE DATABASE 2/2
                # ============================================
                await clearner_database()
