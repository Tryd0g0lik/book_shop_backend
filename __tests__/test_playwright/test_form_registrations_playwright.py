    ...
    async def test_form_registrations_playwright(self, user_registrate_playwright):
        ...
        async with client.get_page_acontext_manager(log_t) as page:
            try:
                ...
                await page.screenshot(path=f"{pathname_local}{pathname_of_screenshort}_full_form_{date_screen}_userID_{id}.png", timeout=1000)
                ...
                try:
                    ...
                    if error_message:
                        ...
                        assert TEST_FORM_DATA[id]["expected"]["valid"] in (True,), "It was checked and data is correct."
                except TimeoutError:
                    assert TEST_FORM_DATA[id]["expected"]["valid"] in (False,), "It was checked and data is correct."
                    ...
                    field_name = TEST_FORM_DATA[id]["expected"]["error_field"]
                    assert result == field_name, f"The name of the form's field where we see data is not correct: {field_name}"
            finally:
